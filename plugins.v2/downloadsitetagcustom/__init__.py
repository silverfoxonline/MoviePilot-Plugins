...
class DownloadSiteTagCustom(_PluginBase):
    # 插件名称
    plugin_name = "下载任务分类与标签-改版"
    # 插件描述
    plugin_desc = "自动给下载任务分类，并将指定站点名称统一为自定义标签"
    # 插件图标
    plugin_icon = "Youtube-dl_B.png"
    # 插件版本
    plugin_version = "2.6.1"
    # 插件作者
    plugin_author = "silverfoxonline（基于叮叮当 2.6）"
    # 作者主页
    author_url = "https://github.com/silverfoxonline/MoviePilot-Plugins"
    # 插件配置项ID前缀
    plugin_config_prefix = "DownloadSiteTagCustom_"
    # 加载顺序
    plugin_order = 2
    # 可使用的用户级别
    auth_level = 1
    # 日志前缀
    LOG_TAG = "[DownloadSiteTagCustom] "

    # MoviePilot 站点显示名到用户习惯标签的映射（大小写不敏感）
    SITE_TAG_ALIASES = {
        "aither": "AITHER",
        "beyondhd": "BHD",
        "orpheus": "OPS",
    ...
    def _generate_site_tag(self, site_name):
        ...
        if not site_name:
            return ""
        site_name = self.SITE_TAG_ALIASES.get(site_name.strip().lower(), site_name)
        if self._site_prefix:
